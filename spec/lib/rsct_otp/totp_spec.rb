...
RSpec.describe RSCT_OTP::TOTP do
  let(:now)   { TEST_TIME }
  let(:token) { TEST_TOKEN }
  let(:totp)  { RSCT_OTP::TOTP.new 'JBSWY3DPEHPK3PXP' }
  ...
  describe '#at' do
    ...
    context 'RFC compatibility' do
      let(:totp) { RSCT_OTP::TOTP.new('GEZDGNBVGY3TQOJQGEZDGNBVGY3TQOJQ') }
  ...
  describe '#verify' do
    ...
    context 'RFC compatibility' do
      let(:totp) { RSCT_OTP::TOTP.new 'wrn3pqx5uqxqvnqr' }
  ...
  describe '#provisioning_uri' do
    ...
    context "with space in name" do
      ...
      it "has correct format" do
        expect(uri).to eq "otpauth://totp/mark%20percival?secret=JBSWY3DPEHPK3PXP"
      end
    end
    ...
    context 'with non-default digits' do
      let(:totp)  { RSCT_OTP::TOTP.new 'JBSWY3DPEHPK3PXP', digits: 8 }
    ...
    context 'with spaces in issuer' do
      let(:totp)  { RSCT_OTP::TOTP.new 'JBSWY3DPEHPK3PXP', issuer: 'Foo Co' }

      it 'includes the uri encoded issuer as parameter' do
        expect(uri).to eq "otpauth://totp/Foo%20Co:mark@percival?secret=JBSWY3DPEHPK3PXP&issuer=Foo%20Co"
      end
    end

    context 'with custom interval' do
      let(:totp)  { RSCT_OTP::TOTP.new 'JBSWY3DPEHPK3PXP', interval: 60 }
    ...
    context 'with custom digest' do
      let(:totp)  { RSCT_OTP::TOTP.new 'JBSWY3DPEHPK3PXP', digest: 'sha256' }
  ...
  describe '#now' do
    ...
    context 'Dropbox 26 char secret output' do
      let(:totp) { RSCT_OTP::TOTP.new 'tjtpqea6a42l56g5eym73go2oa' }
